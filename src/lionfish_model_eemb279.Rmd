...
# Maranhão
t_m = log(25)  # Moura et al (2016)

# Fernando de Noronha
t_fn = log(24) # Prazeres et al (2012)

# Rio de Janeiro
t_rj = log(23) # Lima & Coutinho (2016)

# Northeast Florida
t_fl = log(22.5) # based on monthly averages for Jacksonville obtained from https://seatemperature.info/jacksonville-water-temperature.html 
```
...
lf_sim_fl_long <- lf_sim_fl %>% 
  select(start_l_fl, l_1_fl, l_2_fl, l_3_fl, l_4_fl) %>% 
...
lf_sim_m_long <- lf_sim_m %>% 
  select(start_l_m, l_1_m, l_2_m, l_3_m, l_4_m) %>% 
...
lf_sim_fn_long <- lf_sim_fn %>% 
  select(start_l_fn, l_1_fn, l_2_fn, l_3_fn, l_4_fn) %>% 
...
lf_sim_rj_long <- lf_sim_rj %>% 
  select(start_l_rj, l_1_rj, l_2_rj, l_3_rj, l_4_rj) %>% 
