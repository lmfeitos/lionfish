...
c <- 0.61 # intensity of seasonal growth oscillation. Varies from 0 to 1, with zero being no seasonality and one extreme seasonality
...
```{r}
# test creating a function for the model instead of a forloop
lionfish_gr <- function(l_t) {
  l_t <- l_inf * (1 - exp(- k * (tset - t_0) + (c * k/ 2 * pi) * sin(pi) * (tset - t_s) - (c * k / 2 * pi) * sin(pi) * (t_s))) # formula 8 of the paper
}

# subset lionfish data to include only observations from april 2013
lionfish_ap2013 <- lionfish_long %>% 
  filter(month == "april") %>% 
  filter(year == "2013")
  

lionfish_gr(lionfish_ap2013$frequency)
```
...
for(i in 2:length(tset)) { 
  ...
  l_t <- l_inf * (1 - exp(- k * (tset[i] - t_0) + (c * k/ 2 * pi) * sin(pi) * (tset[i] - t_s) - (c * k / 2 * pi) * sin(pi) * (t_s))) * dt # formula 8 of the paper
