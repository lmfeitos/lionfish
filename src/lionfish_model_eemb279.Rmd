...
Try a loop for different t:

```{r}
tset <- seq(0, 10, length = 10)
Lset <- NaN*tset
...
for(i in 1:length(tset)){
  t <- tset[i]
  dt <- tset[i]- tset[i-1]
  # need to rewrite L as dt?
  L = 448*(1-exp^(-0.47*t+((0.61*0.47/2*3.1415)*sin(3.1415)*(t-0.71))-(0.61*0.47/2*3.1415)*sin(3.14159)*0.71))
  Lset[i] <- L
}
```
