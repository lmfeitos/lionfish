...
tset <- seq(from = 0, to = 4, length.out = 1000)
...
Now need to try to use the growth to estmiate the population structure

```{r}
# Set up new, simpler model for testing
tset <- seq(from = 0, to = 4, length.out = 50)
lf_simple <- data.frame(tset, lengths = NaN*tset)

for(i in 1:length(tset)){
  t <- tset[i]
  l_t <- l_inf*(1-exp(-k*(t-t_0)+(((c*k)/(2*pi))*sinpi(t-t_s))-(((c*k)/(2*pi))*sinpi(t_s))))
  lf_simple$lengths[i] <- l_t
}


for(i in 1:length(lf_simple$lengths)){
  L <- lf_simple$lengths[i]

  dist <- dnorm(lf$tset, mean = L, sd = 26.47)
  n <- 
}

N_t   = 
n = N_t * P_at * P *(bounds | dnorm(L_t, sd))
