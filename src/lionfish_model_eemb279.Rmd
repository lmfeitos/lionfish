...
```{r for loop for florida, echo = FALSE}
...
# Works!
flo <- ggplot(data = lf_sim_fl_long) +
...
```{r for loop for maranhao, echo = FALSE}
...
```{r for loop for noronha, echo = FALSE}
...
As shown in figures 2 and 3, by including average water temperature as a parameter for each different area this model extension estimates that lionfish individuals are more likely to show a skewed frequency towards higher sizes more quickly (Figure 2). This is because living in environments with higher temperatures than the ones found in Northeast Florida can cause fish to grow at a faster rate. This is clearly shown in Figure 3. However, it is necessary to state that these growth patterns are applied from the population of lionfish sampled in Northeast Florida on April 2013, and thus conclusions for other areas are likely to be somewhat different due to inherent baseline data differences. 
