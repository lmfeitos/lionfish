...
The authors used maximum likelihood approach to fit a predicted distribution to the length-frequency histograms from samples of collected lionfish. Length is a proxy for age, so the model estimates the proportion of fish of a given size in each age class. This function was then use to estimate the parameters for the best-fit model:
...
```
***Figure 1.*** Length-frequency histograms for lionfish sampled across three years - 2013 (left), 2014 (middle), and 2015 (right). There were two sampling months for each year, in April and August. These were generated using the raw length data from all samples, as supplied in Johnson and Swenarton 2016.
...
***Figure 2.*** Estimated age-legnth relationship for lionfish in Northeast Florida using the seasonalized von-Bertalanffy growth equation, comparing different values for the intensity of the seasonal growth oscillation. The best-fit model (green, dot-dash) shows moderate seasonality at C = 0.61. For comparison, the model was also evaluated with C = 0 (red, solid) indicating no seasonality, and C = 1 (blue, dashed) indicating complete seasonality.


We also used the original length-frequency data from April 2013 to demonstrate how the snapshot age structure of the population would grow over time, assuming no individuals entered or exited the population (no births, immigration, deaths, or emigration). 
...
Unlike other studies that did age and growth analyses for lionfish in the Caribbean Sea (Akins et al 2014; Rodríguez-Cortés, 2015), Johnson & Swenarton (2016) demonstrated that lionfish growth has suffers substantially from seasonality, at least in a temperate region as Northeast Florida. However, given the localized nature of this study, it is not possible to draw conclusions about its growth patterns for the whole Caribbean area, especially given that southern Caribbean is less affected by seasonality. Nevertheless, Kimball et al (2004) studied the thermal tolerance of lionfish in the Atlantic and demonstrated that they die at 10ºC and can no longer feed at temperatures of 16ºC. Therefore, temperature could be considered a limiting factor for the invasion of areas further north of Florida where water temperatures might reach between 10 and 16ºC (Ruiz-Carus et al 2006).      
...
Johnson, E. G., & Swenarton, M. K. (2016). Age, growth and population structure of invasive lionfish (Pterois volitans/miles) in northeast Florida using a length-based, age-structured population model. PeerJ, 4, e2730.
...
Ruiz-Carus, R., Matheson Jr, R. E., Roberts Jr, D. E., & Whitfield, P. E. (2006). The western Pacific red lionfish, Pterois volitans (Scorpaenidae), in Florida: Evidence for reproduction and parasitism in the first exotic marine fish established in state waters. Biological Conservation, 128(3), 384-390.
